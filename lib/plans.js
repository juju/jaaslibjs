...
plans.prototype = {
  ...
  */
  showActivePlan: function(modelUUID, applicationName, callback) {
    if (!modelUUID) {
      callback('no modelUUID provided, cannot fetch active plan', null, []);
      return;
    }
  ...
  */
  authorizeSLA: function(sla, model, budget, callback) {
    ...
    return this.bakery.post(url, headers, body, utils._wrap(callback));
  },
  ...
  */
  getKpiMetrics: function(charmId, filters, callback) {
    ...
    const qs = utils.serializeObject(payload);
    url += '?' + qs;
    const headers = null;
    return this.bakery.get(
      url, headers, utils._wrap(handler, {parseJSON: true}));
